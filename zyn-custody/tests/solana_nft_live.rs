...
fn a_mirrored_nft_is_deposited_seen_and_paid_back() {
    ...
    let mint_b58 = created
        .lines()
        .find_map(|l| l.strip_prefix("Creating token "))
        .unwrap()
        .split_whitespace()
        .next()
        .unwrap()
    ...
    assert_eq!(
        observed.balance_of([7; 32], 0),
        Some(Fixed::whole(1)),
        "the vault holds the item"
    );
    ...
    run(
        ...
        &[
            ...
            "-s",
            "-o",
            recipient_kp.to_str().unwrap(),
        ],
    );
    let recipient = pubkey(&run(
        "solana",
        &["address", "-k", recipient_kp.to_str().unwrap()],
    ))
    .unwrap();
    let rpc = Rpc::connect_unchecked(URL, Cluster::Devnet);
    for _ in 0..40 {
    ...
    let groups = chunk(
        ...
        &[SolPayout::token(recipient, mint, 1)],
    )
    .unwrap();
    ...
    for _ in 0..60 {
        ...
        if st.contains("Confirmed") || st.contains("Finalized") {
            break;
        }
    ...
    let left = run(
        "spl-token",
        &["balance", &mint_b58, "--owner", &vault_b58, "-u", URL],
    );
