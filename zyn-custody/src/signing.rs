...
//! one copy of. `Zcash` and `Solana` instantiate it; the type aliases keep the
...
/// The Solana counterpart of [`Quorum`]: one message, one signature, no
/// randomizers. Same two rounds, same discipline.
pub trait SolanaQuorum {
    fn round1(
        &mut self,
        request_id: u64,
        message: &[u8],
        now: u64,
    ) -> BTreeMap<IdentifierFor<Solana>, frost_core::round1::SigningCommitments<Solana>>;

    fn round2(
        &mut self,
        request_id: u64,
        chosen: &[IdentifierFor<Solana>],
        package: &frost_core::SigningPackage<Solana>,
    ) -> BTreeMap<IdentifierFor<Solana>, frost_core::round2::SignatureShare<Solana>>;
}

/// Solana share-holders in this process: the V0 posture and the tests.
pub struct LocalSolanaQuorum {
    participants: Vec<crate::custodian::solana::Participant>,
}
...
impl SolanaQuorum for LocalSolanaQuorum {
    fn round1(
        &mut self,
        request_id: u64,
        message: &[u8],
