...
/// The same discipline for Solana's ed25519 vault, which is simpler: one
/// message, one signature, and no re-randomization — so one nonce per request
/// rather than one per action.
pub mod solana {
    use std::collections::BTreeMap;

    use crate::ceremony::{IdentifierFor, Solana, ThresholdKeys};
    ...
    struct Live {
        message: Vec<u8>,
        session: Option<Session<Solana>>,
        opened_at: u64,
    }

    /// One Solana share, and the requests it has open.
    pub struct Participant {
        keys: Keys,
        live: BTreeMap<u64, Live>,
    }
    ...
    impl Participant {
        ...
        pub fn round1<R: rand_core::RngCore + rand_core::CryptoRng>(
            ...
            now: u64,
            rng: &mut R,
        ) -> Result<frost_core::round1::SigningCommitments<Solana>, CustodyError> {
            self.drop_expired(now);
            if self.live.contains_key(&request_id) {
                return Err(CustodyError::AlreadyOpen);
            }
