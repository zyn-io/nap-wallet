...
pub fn save<C: Ciphersuite>(dir: &Path, keys: &[(IdentifierFor<C>, ThresholdKeys<C>)]) -> std::io::Result<()> {
    ...
    if let Some((_, k)) = keys.first() {
        std::fs::write(dir.join("public.bin"), k.public_package.serialize().map_err(bad)?)?;
    }
    Ok(())
}
...
pub fn load_public<C: Ciphersuite>(dir: &Path) -> std::io::Result<frost_core::keys::PublicKeyPackage<C>> {
    frost_core::keys::PublicKeyPackage::<C>::deserialize(&std::fs::read(dir.join("public.bin"))?).map_err(bad)
}

pub fn load<C: Ciphersuite>(dir: &Path) -> std::io::Result<Vec<(IdentifierFor<C>, ThresholdKeys<C>)>> {
    let public = frost_core::keys::PublicKeyPackage::<C>::deserialize(&std::fs::read(dir.join("public.bin"))?)
        .map_err(bad)?;
    ...
    for name in names {
        ...
        let kp = frost_core::keys::KeyPackage::<C>::deserialize(&std::fs::read(dir.join(&name))?).map_err(bad)?;
        out.push((*kp.identifier(), ThresholdKeys { key_package: kp, public_package: public.clone() }));
    }
    Ok(out)
}
