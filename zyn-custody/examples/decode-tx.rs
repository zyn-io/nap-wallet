...
fn main() {
    ...
    for branch in [BranchId::Nu5, BranchId::Nu6_3] {
        match Transaction::read(&raw[..], branch) {
            Ok(tx) => {
                ...
                println!(
                    "  transparent vout: {}",
                    tx.transparent_bundle().map(|b| b.vout.len()).unwrap_or(0)
                );
                println!(
                    "  orchard actions:  {}",
    ...
    let fvk =
        orchard::keys::FullViewingKey::from_bytes(&unhex(fvk_hex).try_into().unwrap()).unwrap();
    ...
    if let Some(b) = tx.ironwood_bundle() {
        ...
        for (i, a) in b.actions().iter().enumerate() {
            ...
            match zcash_note_encryption::try_note_decryption(&domain, &ivk, a) {
                Some((note, _addr, memo)) => println!(
                    "  ironwood action {}: OURS, {} zat, memo {:?}",
                    i,
                    note.value().inner(),
                    zyn_custody::memo::decode(&memo)
                        .map(|a| a.iter().map(|b| format!("{:02x}", b)).collect::<String>())
                ),
    ...
    match keys.scan_actions(&raw, [0u8; 32], 0) {
        Ok(s) => {
            println!(
                "scanner: {} orchard action(s), {} ours, {} deposit(s)",
                s.actions.len(),
                s.actions.iter().filter(|a| a.ours.is_some()).count(),
                s.deposits.len()
            );
            for d in &s.deposits {
                println!(
