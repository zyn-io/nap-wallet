...
fn main() {
    ...
    println!("eip712 digest      {:>10.0}/sec", n as f64 / t.elapsed().as_secs_f64());
    ...
    println!("secp256k1 recover  {:>10.0}/sec", n as f64 / t.elapsed().as_secs_f64());
    ...
    println!("ed25519 verify     {:>10.0}/sec", n as f64 / t.elapsed().as_secs_f64());
    ...
    for (name, c) in [("secp256k1", evm.clone()), ("ed25519", cred.clone())] {
        let pending: Vec<Pending> =
            (0..n).map(|_| Pending::of::<SwapState>(c.clone(), &auth, &intent)).collect();
        let t = Instant::now();
