...
fn funded_chain() -> VmNode<SwapState> {
    ...
    let mut n: VmNode<SwapState> = VmNode::new(CHAIN, Params::v1(), policy, Economics::flat(10_000));
    let each = Fixed::whole(100);
    let total = Fixed::whole(300);
    n.submit_operator(Intent::AttestVaultBalance { asset: XZEC, observed: total }, 0);
...
fn a_piece_is_claimed_traded_twice_and_redeemed_above_its_claim_price() {
    ...
    node.create_collection(&creator, *b"NAP\0\0\0\0\0", 1, 100).expect("create");
    let collection = node.collections().expect("collections")[0].id;
    node.advance_collection(&creator, collection, 1).expect("open claiming");
    ...
    node.fund_collection(&a, collection, price).expect("buyer pays");
    node.mint_collection_item(&creator, collection, account(&a), *b"NAP\0\0\0\0\0", [7u8; 32]).expect("mint");
    let item = node.assets().expect("assets").iter().find(|x| x.is_item()).map(|x| x.id).expect("an item exists");

    // The market opens; only now is there a floor.
    node.advance_collection(&creator, collection, 2).expect("close claiming");
    node.advance_collection(&creator, collection, 3).expect("list");
    let floor_at_claim = node.collection(collection).unwrap().unwrap().redeem_price;
    assert_eq!(floor_at_claim, price, "one piece, one claim: the floor is what was paid");

    // A rests an offer; B takes it without A being asked again.
    let sale = Fixed::whole(6);
    node.place_offer(&a, item, Fixed::ONE, XZEC, sale, u64::MAX).expect("A lists");
    ...
    node.place_offer(&b, item, Fixed::ONE, XZEC, sale, u64::MAX).expect("B lists");
    let offer = node.offers().expect("book")[0].id;
    node.take_offer(&a, offer).expect("A buys back");

    // Two trades, each charging 1% to both sides, half of every charge to the
    // pool: 4 x 1% of 6, halved, is 0.12 on top of the 5 that was claimed.
    let each = Fixed::raw(sale.0 / 100);
    let to_pool = Fixed::raw(each.add(each).unwrap().0 / 2);
    ...
    let before = node.account(&a).expect("account").expect("a record").spendable.iter().find(|(x, _)| *x == XZEC).map(|(_, v)| *v).unwrap_or(Fixed::ZERO);
    node.redeem_collection_item(&a, item).expect("redeem");
    let after = node.account(&a).expect("account").expect("a record").spendable.iter().find(|(x, _)| *x == XZEC).map(|(_, v)| *v).unwrap_or(Fixed::ZERO);
    assert_eq!(after.sub(before).unwrap(), floor_now, "paid exactly the floor");
    assert_eq!(node.collection(collection).unwrap().unwrap().outstanding, 0, "nothing left outstanding");
}
...
fn a_resting_offer_is_taken_without_the_maker_signing_anything() {
    ...
    node.create_collection(&creator, *b"NAP\0\0\0\0\0", 1, 100).expect("create");
    let collection = node.collections().expect("collections")[0].id;
    node.advance_collection(&creator, collection, 1).expect("minting");
    node.mint_collection_item(&creator, collection, account(&a), *b"NAP\0\0\0\0\0", [3u8; 32]).expect("mint");
    let item = node.assets().expect("assets").iter().find(|x| x.is_item()).map(|x| x.id).expect("item");
    node.advance_collection(&creator, collection, 2).expect("closed");
    node.advance_collection(&creator, collection, 3).expect("live");

    node.place_offer(&a, item, Fixed::ONE, XZEC, Fixed::whole(6), u64::MAX).expect("list");
    ...
    assert!(node.offers().expect("book").is_empty(), "and it leaves the book");

    let held = node.account(&b).expect("account").expect("a record").spendable.iter().any(|(x, v)| *x == item && *v == Fixed::ONE);
