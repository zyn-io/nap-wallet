...
fn a_confirmed_anchor_yields_a_bundle_that_checks_out_end_to_end() {
    ...
    let root = |name: &str| {
        std::fs::read(da.join(format!("chain-21/{}/{}", anchor.checkpoint.epoch, name))).unwrap()
    };
    let a = Anchor::decode(&root("anchor.bin")).unwrap();
    assert_eq!(
        a.id(),
        anchor.id(),
        "anchor.bin hashes to the id the memo carries"
    );
    assert_eq!(a.epochs, 2);
    let p = Published::decode(&root("published.bin")).unwrap();
    p.verify().unwrap();
    assert_eq!(
        p.root, a.checkpoint.state_root,
        "the leaves open the anchored root"
    );
    ...
    assert_eq!(
        out.checkpoints.last(),
        Some(&a.checkpoint),
        "replaying the intents reproduces the anchored checkpoint"
    );
    ...
    let last = bad.len() - 1;
    bad[last] ^= 1;
    assert!(Published::decode(&bad)
        .map(|p| p.verify().is_err())
        .unwrap_or(true));
}
...
fn an_epoch_not_yet_sealed_on_disk_is_refused_rather_than_published() {
    ...
    {
        let n = shared.lock().unwrap();
        publish::bundle_for(&n, &dir, 21, &anchor, "cafe", 4_400_000)
            .expect("a sealed journal publishes");
    }
    ...
    assert!(
        journal::read_epoch(&std::fs::read(&path).unwrap())
            .unwrap()
            .records
            .len()
            < file.records.len()
