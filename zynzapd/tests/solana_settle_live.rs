...
fn run(federated: bool) {
    ...
    let dir = std::env::temp_dir().join(format!("zyn-settle-live-{}-{}", std::process::id(), federated));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();

    // --- the chain: one Solana-vaulted asset, one account with a balance ---
    let mut s = SwapState::new(3, Params::testnet());
    let sol = s.next_asset_id;
    s.next_asset_id += 1;
    s.tokens.insert(sol, TokenInfo::bridged(symbol(b"SOL.zy"), ORIGIN_SOLANA));
    ...
    let node = Arc::new(Mutex::new(Node::resume(s, policy, Economics::flat(10_000), 0)));
    ...
    for f in [&payer, &nonce_kp] {
        assert!(Command::new("solana-keygen").args(["new", "--no-bip39-passphrase", "-f", "-s", "-o"]).arg(f).output().unwrap().status.success());
    }
    cli(&["airdrop", "5", "-k", payer.to_str().unwrap()]);
    cli(&["airdrop", "2", &vault_b58]);
    cli(&["create-nonce-account", nonce_kp.to_str().unwrap(), "0.01", "--nonce-authority", &vault_b58, "-k", payer.to_str().unwrap()]);
    ...
    std::fs::write(&reveals, format!("{} {} {}\n", hex(&ACCOUNT), base58_encode(&dest), hex(&SALT))).unwrap();
    ...
    // --- the user's side on Zyn: credited, released, bound, exiting ---
    {
        ...
        go(&mut n, Intent::AttestVaultBalance { asset: sol, observed: Fixed::whole(2) });
        let index = n.state().next_deposit_index(sol);
        go(&mut n, Intent::CreditDeposit { account: ACCOUNT, asset: sol, amount: Fixed::whole(1), index, external_ref: [1u8; 32] });
        ...
        go(&mut n, Intent::RequestWithdrawal { account: ACCOUNT, asset: sol, amount: Fixed::whole(1), destination });
        assert_eq!(n.state().balance(&ACCOUNT, sol), Fixed::ZERO, "units left the balance on request");
        assert_eq!(n.state().accounts[&ACCOUNT].pending[&sol].amount, Fixed::whole(1));
    }
    ...
    let public = zyn_custody::shares::load_public::<zyn_custody::ceremony::Solana>(&dir.join("shares")).unwrap();
    let mut settler = SolanaSettler::new(rpc, held, public, custodians, 2, &nonce_b58, load_reveals(&reveals).unwrap(), sol, dir.clone(), 3).unwrap();
    ...
    assert_eq!(lamports_of(&base58_encode(&dest)) - dest_before, 1_000_000_000 - 5_000);
    ...
    let pending = n.state().accounts.get(&ACCOUNT).and_then(|a| a.pending.get(&sol)).map(|p| p.amount);
    assert!(pending.map(|p| p.is_zero()).unwrap_or(true), "still pending: {:?}", pending);
    assert_eq!(n.state().backing_of(sol), Fixed::ZERO, "backing was not released");
    ...
    assert_eq!(zynzapd::settle::Ledger::load(&dir, 3, sol).unwrap().entries.len(), 1);
}
