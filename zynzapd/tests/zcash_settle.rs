...
use zynzapd::settle::{load_zcash_reveals, zcash_commitment, Ledger, Status, ZcashDestination, ZcashSettler};
        ...
        for stream in listener.incoming() {
            ...
            loop {
                ...
                if let Some(split) = text.find("\r\n\r\n") {
                    let want: usize = text[..split].lines().find_map(|l| l.strip_prefix("Content-Length: ")).and_then(|v| v.trim().parse().ok()).unwrap_or(0);
                    if raw.len() >= split + 4 + want { break; }
                }
            }
            ...
            let result = match method {
                ...
                "sendrawtransaction" => {
                    *captured.lock().unwrap() = body["params"][0].as_str().unwrap_or("").to_string();
                    sent.store(true, Ordering::SeqCst);
                    "\"deadbeef\"".to_string()
                }
                "getrawtransaction" if sent.load(Ordering::SeqCst) => r#"{"confirmations":6,"height":4300001}"#.to_string(),
                "getrawtransaction" => {
...
fn t_address(hash: [u8; 20]) -> (TransparentAddress, String) {
    ...
    (TransparentAddress::PublicKeyHash(hash), zyn_custody::solana::base58_encode(&[&body[..], &chk[..4]].concat()))
}
fn hex(b: &[u8]) -> String {
...
fn funded(vault_addr: orchard::Address, version: NoteVersion) -> Arc<Mutex<NoteStore>> {
    ...
    let other = {
        ...
        Note::from_parts(ofvk.address_at(0u32, Scope::External), NoteValue::from_raw(1), rho, RandomSeed::from_bytes([7u8; 32], &rho).unwrap(), version).unwrap()
    };
    store.append(&ExtractedNoteCommitment::from(other.commitment()), false);
    let rho = Rho::from_bytes(&[5u8; 32]).unwrap();
    let note = Note::from_parts(vault_addr, NoteValue::from_raw(10_000_000), rho, RandomSeed::from_bytes([6u8; 32], &rho).unwrap(), version).unwrap();
    let pos = store.append(&ExtractedNoteCommitment::from(note.commitment()), true).unwrap();
...
fn run(pool: ValuePool) {
    ...
    let keys: Vec<_> = Ceremony::new(2, 3).unwrap().run_for::<Zcash, _>(&mut rand::rngs::OsRng).unwrap().into_iter().collect();
    shares::save::<Zcash>(&dir.join("shares"), &keys).unwrap();
    let group = keys[0].1.group_key();
    let fvk: FullViewingKey = (0u8..64).find_map(|n| orchard_viewing_key(&group, [n; 32], [n; 32])).unwrap();
    ...
    let notes = match pool {
        ValuePool::Orchard => PoolStores { orchard: funded(vault_addr, NoteVersion::V2), ironwood: empty() },
        ValuePool::Ironwood => PoolStores { orchard: empty(), ironwood: funded(vault_addr, NoteVersion::V3) },
    };
    ...
    let (dest, dest_str) = match pool {
        ...
        ValuePool::Ironwood => {
            ...
            let ua = Address::try_from_items(vec![Receiver::Orchard(a.to_raw_address_bytes())]).unwrap();
            (ZcashDestination::Shielded(a), ua.encode(&zcash_protocol::consensus::NetworkType::Test))
        }
    };
    let destination = zcash_commitment(&dest, &SALT);
    let reveals = dir.join("reveals");
    std::fs::write(&reveals, format!("{} {} {}\n", hex(&ACCOUNT), dest_str, hex(&SALT))).unwrap();

    // --- the user's side on Zyn ---
    let policy = EpochPolicy { intents_per_epoch: 10_000, epochs_per_anchor: 10_000, max_seconds_per_epoch: 0, max_seconds_per_anchor: 0 };
    let node: Arc<Mutex<Node<SwapState>>> = Arc::new(Mutex::new(Node::new(3, Params::testnet(), policy, Economics::flat(10_000))));
    {
        let mut n = node.lock().unwrap();
        let go = |n: &mut Node<SwapState>, i: Intent| {
            let step = n.submit_operator(i, 0);
            assert!(!step.rejected(), "rejected at seq {}: {:?}", step.seq, step.receipts);
        };
        go(&mut n, Intent::AttestVaultBalance { asset: XZEC, observed: Fixed::raw(10_000_000 * ZAT) });
        let index = n.state().next_deposit_index(XZEC);
        go(&mut n, Intent::CreditDeposit { account: ACCOUNT, asset: XZEC, amount: Fixed::raw(5_000_000 * ZAT), index, external_ref: [1u8; 32] });
        let epoch = n.state().epoch();
        go(&mut n, Intent::Checkpoint);
        go(&mut n, Intent::ConfirmAnchor { epoch });
        go(&mut n, Intent::BindWithdrawal { account: ACCOUNT, destination });
        go(&mut n, Intent::RequestWithdrawal { account: ACCOUNT, asset: XZEC, amount: Fixed::raw(2_000_000 * ZAT), destination });
    }
    ...
    let zebra = Zebra::connect(&addr.ip().to_string(), addr.port(), None, ZebraNet::Testnet).unwrap();
    let mut settler = ZcashSettler::new(
        zebra, Network::TestNetwork, shares::load::<Zcash>(&dir.join("shares")).unwrap(), 2, fvk.clone(), notes.clone(),
        load_zcash_reveals(&reveals, Network::TestNetwork).unwrap(), XZEC, 6, dir.clone(), 3,
        shares::load_public::<Zcash>(&dir.join("shares")).unwrap(), None,
    ).unwrap();
    ...
    assert_eq!(settler.poll_once(&node, 0).unwrap(), 1, "the exit was not confirmed on Zyn");
    ...
    let bytes: Vec<u8> = (0..raw.len() / 2).map(|i| u8::from_str_radix(&raw[i * 2..i * 2 + 2], 16).unwrap()).collect();
    let env = payout::Envelope::testnet_at(TIP as u32);
    let tx = zcash_primitives::transaction::Transaction::read(&bytes[..], env.branch).expect("a transaction");
    assert_eq!(env.branch, BranchId::Nu6_3, "testnet at this height is NU6.3");
    ...
    match pool {
        ValuePool::Orchard => {
            ...
            o.verify_proof(&payout::verifying_key(version)).expect("the proof verifies");
            ...
            assert_eq!(ZcashDestination::Transparent(t.vout[0].recipient_address().unwrap()), dest);
            assert_eq!(i64::from(*o.value_balance()), 2_000_000, "exactly the burned amount leaves the pool");
        }
        ValuePool::Ironwood => {
            ...
            b.verify_proof(&payout::verifying_key(version)).expect("the proof verifies");
            let ivk = orchard::keys::PreparedIncomingViewingKey::new(&their_fvk.to_ivk(Scope::External));
            let got: Vec<u64> = b.actions().iter().filter_map(|a| zcash_note_encryption::try_note_decryption(&orchard::note_encryption::IronwoodDomain::for_action(a), &ivk, a)).map(|(n, _, _)| n.value().inner()).collect();
            let fee = payout::zip317_fee(b.actions().len(), 0);
            assert_eq!(got, vec![2_000_000 - fee], "the recipient receives the exit less the fee it pays");
    ...
    let pending = n.state().accounts.get(&ACCOUNT).and_then(|a| a.pending.get(&XZEC)).map(|p| p.amount);
    assert!(pending.map(|p| p.is_zero()).unwrap_or(true), "still pending: {:?}", pending);
    assert_eq!(n.state().backing_of(XZEC), Fixed::raw(3_000_000 * ZAT), "backing was not released");
    assert_eq!(notes.of(pool).lock().unwrap().balance(), 0, "the spent note is still held");
    let ledger = Ledger::load(&dir, 3, XZEC).unwrap();
    assert_eq!(ledger.entries.len(), 1);
    assert_eq!(ledger.entries[0].status, Status::Confirmed);
    assert_eq!(ledger.entries[0].id, hex(&{ let mut t = *tx.txid().as_ref(); t.reverse(); t }));
    drop(n);
    assert_eq!(settler.poll_once(&node, 0).unwrap(), 0, "a second pass had nothing to do");
}
