...
fn policy() -> EpochPolicy {
    EpochPolicy { intents_per_epoch: 10, epochs_per_anchor: 1, max_seconds_per_epoch: 0, max_seconds_per_anchor: 0 }
}
...
fn a_restart_keeps_the_lineage_and_the_journal_reproduces_the_state() {
    let dir = tmp("lineage");
    let first = boot::open_at(&dir, 9, Params::testnet(), policy(), Economics::flat(10_000), 0, false, &dir.join("da")).unwrap();
    ...
    let shared = Arc::new(Mutex::new(first.node));
    {
        ...
        for i in 0..12u8 {
            ...
            assert!(!step.unrecorded && !step.rejected(), "deposit {} failed: {:?}", i, step.receipts);
        }
        n.seal_now(1);
        n.anchor_now(1);
    ...
    let second = boot::open_at(&dir, 9, Params::testnet(), policy(), Economics::flat(10_000), 5, false, &dir.join("da")).unwrap();
    assert!(second.resumed);
    assert_eq!(second.node.ledger().len(), 2);
    assert_eq!(second.node.ledger().head_root(), head, "the lineage did not survive the restart");
    ...
    let epochs: Vec<_> = files.iter().map(|(_, b)| journal::read_epoch(b).unwrap()).collect();
    let out = journal::replay(SwapState::new(9, Params::testnet()), &epochs).unwrap();
    assert_eq!(out.state.state_root(), root, "the journal does not reproduce the saved state");
...
fn a_ledger_without_a_state_and_a_stale_state_are_both_refused() {
    let dir = tmp("stale");
    let b = boot::open_at(&dir, 9, Params::testnet(), policy(), Economics::flat(10_000), 0, false, &dir.join("da")).unwrap();
    ...
    b.store.save_ledger(shared.lock().unwrap().ledger()).unwrap();
    let err = boot::open_at(&dir, 9, Params::testnet(), policy(), Economics::flat(10_000), 0, false, &dir.join("da")).map(|_| ()).unwrap_err();
