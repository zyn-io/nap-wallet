...
use zynzapd::wallet::{network_name, unit, Event, Wallet, WalletBackup, CONFIRMATIONS, ZAT_PER_ZEC};
...
fn read_secret(path: &str) -> Result<String, String> {
    ...
    if path == "-" {
        std::io::stdin().read_to_string(&mut out).map_err(|e| e.to_string())?;
    } else {
...
fn log(e: Event) {
    match e {
        Event::Progress { at, to } => { if at.is_multiple_of(1000) || at == to { eprintln!("  … {} of {}", at, to) } }
        Event::Received { pool, zatoshi, height, memo, .. } => eprintln!("  received {:>10} zat  in {:?} at height {}{}", zatoshi, pool, height, if memo.is_empty() { String::new() } else { format!("  memo: {}", memo) }),
        Event::Spent { pool, zatoshi, from_height, .. } => eprintln!("  spent  {:>12} zat  ({:?} note from height {})", zatoshi, pool, from_height),
    }
}
...
fn report(w: &Wallet, synced: u64) {
    ...
    println!("balance: {:.8} {}  (Ironwood {} zat, Orchard {} zat)", (o + i) as f64 / ZAT_PER_ZEC, unit(w.network()), i, o);
}
    ...
    let r: Result<(), String> = (|| {
        match (a.get(1).map(String::as_str).unwrap_or(""), a.len()) {
            ("new", 3) => {
                ...
                eprintln!("{} wallet, birthday {}", network_name(w.network()), w.state.birthday);
                println!("{}", w.address());
            }
            ("import", 5) => {
                let seed: [u8; 32] = (0..32).map(|i| u8::from_str_radix(a[3].get(i * 2..i * 2 + 2).unwrap_or("zz"), 16).ok()).collect::<Option<Vec<u8>>>().and_then(|v| v.try_into().ok()).ok_or("key is 64 hex characters")?;
            ...
            ("import-mnemonic", 5) | ("import-mnemonic", 6) => {
                ...
                let birthday: u64 = a[4].parse().map_err(|_| "birthday is a block height")?;
                let account: u32 = a.get(5).map(String::as_str).unwrap_or("0").parse().map_err(|_| "account is a number below 2^31")?;
                let passphrase = std::env::var("ZYN_BIP39_PASSPHRASE").unwrap_or_default();
                let w = Wallet::import_mnemonic(&a[2], &words, &passphrase, account, birthday, client())?;
                eprintln!("{} wallet, birthday {}, ZIP-32 account {}", network_name(w.network()), w.state.birthday, account);
            ...
            ("reset", 3) | ("reset", 4) => {
                ...
                let birthday = match a.get(3) { Some(h) => h.parse().map_err(|_| "birthday is a block height")?, None => w.state.birthday };
                w.reset(birthday)?;
                eprintln!("{} state reset to birthday {}; the next sync rescans from there", network_name(w.network()), w.state.birthday);
            }
            ("sync", 3) => {
                let mut w = Wallet::open(&a[2], client())?;
            ...
            ("send", 5) | ("send", 6) => {
                ...
                eprintln!("sent; the change shows up after {} confirmations", CONFIRMATIONS);
                println!("{}", txid);
            }
            _ => {
