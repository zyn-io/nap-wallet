...
fn key(path: &str) -> SigningKey {
    ...
    let seed: [u8; 32] = bytes.try_into().unwrap_or_else(|_| die("a key file is 32 bytes"));
...
fn call(addr: &str, frame: &[u8]) -> Vec<u8> {
    let mut s = TcpStream::connect(addr).unwrap_or_else(|e| die(&format!("cannot reach {}: {}", addr, e)));
    s.write_all(&(frame.len() as u32).to_be_bytes()).unwrap();
    s.write_all(frame).unwrap();
    let mut len = [0u8; 4];
    s.read_exact(&mut len).unwrap_or_else(|_| die("no reply"));
    let mut body = vec![0u8; u32::from_be_bytes(len) as usize];
    s.read_exact(&mut body).unwrap_or_else(|_| die("truncated reply"));
    if body.first() == Some(&wire::STATUS_ERR) {
        let n = u16::from_be_bytes([body[1], body[2]]) as usize;
        die(&format!("node refused: {}", String::from_utf8_lossy(&body[3..3 + n])));
    }
    body[1..].to_vec()
}
...
fn status(addr: &str, chain: u32) -> Status {
    ...
    if let Ok(n) = d.u32() {
        for _ in 0..n {
            ...
            let err = d.u16().ok().and_then(|l| d.take_bytes(l as usize).ok()).map(|b| String::from_utf8_lossy(b).to_string()).unwrap_or_default();
...
fn submit(addr: &str, chain: u32, k: &SigningKey, intent: Intent) {
    ...
    println!("accepted: seq {} epoch {} ({} receipt(s))", seq, epoch, count);
}
...
fn fixed_of(s: &str) -> Fixed {
    ...
    let frac: i128 = if frac.is_empty() { 0 } else { format!("{:0<18}", frac).parse().unwrap_or_else(|_| die("amount")) };
    Fixed::raw(whole * 1_000_000_000_000_000_000 + frac)
}

fn dest(t: &str, salt: &str) -> ([u8; 32], ZcashDestination) {
    let a = parse_destination(t, Network::TestNetwork).unwrap_or_else(|| die("not a testnet transparent or unified address"));
...
fn main() {
    ...
    match (cmd, a.len()) {
        ...
        ("balance", 5) => {
            ...
            let mut e = Encoder::new();
            e.u8(OP_ACCOUNT).u32(chain).bytes(&id).u64(epoch).u8(Scheme::Ed25519.tag()).bytes(k.verifying_key().as_bytes()).bytes(&sig);
            ...
            for _ in 0..n {
                let (asset, amount, since) = (d.u32().unwrap_or(0), d.fixed().unwrap_or(Fixed::ZERO), d.u64().unwrap_or(0));
                println!("  exiting    asset {:<3} {}  (requested epoch {})", asset, amount, since);
            }
            let n = d.u32().unwrap_or(0);
            for _ in 0..n {
                let (asset, amount, epoch) = (d.u32().unwrap_or(0), d.fixed().unwrap_or(Fixed::ZERO), d.u64().unwrap_or(0));
                println!("  unreleased asset {:<3} {}  (credited epoch {}, spendable once anchored)", asset, amount, epoch);
            }
            if let Ok(dest) = d.hash() {
                let bound = dest != [0u8; 32];
                println!("  binding    {}", if bound { hex(&dest) } else { "none".to_string() });
                if let Ok(1) = d.u8() {
        ...
        ("bind-sol", 7) => {
            let chain: u32 = a[3].parse().unwrap_or_else(|_| die("chain id"));
            let k = key(&a[4]);
            let addr = zyn_custody::solana::pubkey(&a[5]).unwrap_or_else(|| die("not a Solana address"));
            let destination = zyn_bridge::solana::commitment(&addr, &unhex32(&a[6]));
            submit(&a[2], chain, &k, Intent::BindWithdrawal { account: account(&k), destination });
        }
        ("withdraw-sol", 9) => {
            let chain: u32 = a[3].parse().unwrap_or_else(|_| die("chain id"));
            let k = key(&a[4]);
            let asset: u32 = a[5].parse().unwrap_or_else(|_| die("asset id"));
            let sol: f64 = a[6].parse().unwrap_or_else(|_| die("amount in SOL"));
            let lamports = (sol * 1e9).round() as i128;
            let addr = zyn_custody::solana::pubkey(&a[7]).unwrap_or_else(|| die("not a Solana address"));
            let destination = zyn_bridge::solana::commitment(&addr, &unhex32(&a[8]));
            submit(&a[2], chain, &k, Intent::RequestWithdrawal { account: account(&k), asset, amount: Fixed::raw(lamports * 1_000_000_000), destination });
        }
        ("withdraw-item", 8) => {
            ...
            let k = key(&a[4]);
            let asset: u32 = a[5].parse().unwrap_or_else(|_| die("asset id"));
            let addr = zyn_custody::solana::pubkey(&a[6]).unwrap_or_else(|| die("not a Solana address"));
            let destination = zyn_bridge::solana::commitment(&addr, &unhex32(&a[7]));
            submit(&a[2], chain, &k, Intent::RequestWithdrawal { account: account(&k), asset, amount: Fixed::whole(1), destination });
        }
        ("reveal-sol", 5) => {
            let k = key(&a[2]);
        ...
        ("pools", 4) => {
            ...
            for _ in 0..n {
                let (id, a0, a1) = (d.u32().unwrap_or(0), d.u32().unwrap_or(0), d.u32().unwrap_or(0));
                let (r0, r1) = (d.fixed().unwrap_or(Fixed::ZERO), d.fixed().unwrap_or(Fixed::ZERO));
                let fee = d.u16().unwrap_or(0);
                let has_ref = d.u8().unwrap_or(0) != 0;
                let (rp, rs, eff) = (d.fixed().unwrap_or(Fixed::ZERO), d.u64().unwrap_or(0), d.u16().unwrap_or(fee));
                println!("pool {:<3} asset {} : asset {}   reserves {} : {}   fee {} bps{}", id, a0, a1, r0, r1, fee, if has_ref { format!("   reference {} (seq {}), charging {} bps", rp, rs, eff) } else { String::new() });
            }
        }
        ("quote", 7) => {
            let chain: u32 = a[3].parse().unwrap_or_else(|_| die("chain id"));
            let asset_in: u32 = a[4].parse().unwrap_or_else(|_| die("asset in"));
            let path: Vec<u32> = a[5].split(',').map(|p| p.parse().unwrap_or_else(|_| die("pool id"))).collect();
            let amount = fixed_of(&a[6]);
            let mut e = Encoder::new();
            e.u8(wire::OP_QUOTE).u32(chain).u32(asset_in).u32(path.len() as u32);
            for p in &path {
                e.u32(*p);
            }
            e.i128(amount.0);
            ...
            let asset_out = d.u32().unwrap_or(0);
            println!("{} of asset {} -> {} of asset {}  (best case {}, a perfectly netted batch)", amount, asset_in, out, asset_out, best);
        }
        ("create-pool", 9) | ("create-pool", 10) => {
            let chain: u32 = a[3].parse().unwrap_or_else(|_| die("chain id"));
            let k = key(&a[4]);
            let asset_a: u32 = a[5].parse().unwrap_or_else(|_| die("asset a"));
            let asset_b: u32 = a[7].parse().unwrap_or_else(|_| die("asset b"));
            let fee_bps: u16 = a.get(9).map(|f| f.parse().unwrap_or_else(|_| die("fee bps"))).unwrap_or(30);
            submit(&a[2], chain, &k, Intent::CreatePool { creator: account(&k), asset_a, asset_b, amount_a: fixed_of(&a[6]), amount_b: fixed_of(&a[8]), fee_bps });
        }
        ("remove-liquidity", 7) => {
            let chain: u32 = a[3].parse().unwrap_or_else(|_| die("chain id"));
            let k = key(&a[4]);
            let pool: u32 = a[5].parse().unwrap_or_else(|_| die("pool"));
            submit(&a[2], chain, &k, Intent::RemoveLiquidity { account: account(&k), pool, shares: fixed_of(&a[6]), min0: Fixed::ZERO, min1: Fixed::ZERO });
        }
        ("add-liquidity", 8) => {
            let chain: u32 = a[3].parse().unwrap_or_else(|_| die("chain id"));
            let k = key(&a[4]);
            let pool: u32 = a[5].parse().unwrap_or_else(|_| die("pool"));
            submit(&a[2], chain, &k, Intent::AddLiquidity { account: account(&k), pool, max0: fixed_of(&a[6]), max1: fixed_of(&a[7]), min_shares: Fixed::ZERO });
        }
        ("swap", 9) => {
            let chain: u32 = a[3].parse().unwrap_or_else(|_| die("chain id"));
            let k = key(&a[4]);
            let asset_in: u32 = a[5].parse().unwrap_or_else(|_| die("asset in"));
            let path: Vec<u32> = a[6].split(',').map(|p| p.parse().unwrap_or_else(|_| die("pool id"))).collect();
            submit(&a[2], chain, &k, Intent::SwapExactIn { account: account(&k), asset_in, path, amount_in: fixed_of(&a[7]), min_out: fixed_of(&a[8]) });
        }
        ("transfer", 8) => {
            let chain: u32 = a[3].parse().unwrap_or_else(|_| die("chain id"));
            let k = key(&a[4]);
            let asset: u32 = a[6].parse().unwrap_or_else(|_| die("asset"));
            submit(&a[2], chain, &k, Intent::Transfer { from: account(&k), to: unhex32(&a[5]), asset, amount: fixed_of(&a[7]) });
        }
        ("shield", 5) => {
            use rand::RngCore;
            let chain: u32 = a[3].parse().unwrap_or_else(|_| die("chain id"));
            ...
            submit(&a[2], chain, &k, Intent::Reblind { account: account(&k), blind });
        ...
        ("mint-item", 9) => {
            ...
            if b.is_empty() || b.len() > 8 {
                die("symbol is 1 to 8 bytes");
            }
            symbol[..b.len()].copy_from_slice(b);
            let supply: i64 = a[6].parse().unwrap_or_else(|_| die("supply, whole units"));
            submit(&a[2], chain, &k, Intent::MintItem { creator: account(&k), symbol, supply: Fixed::whole(supply), bond: fixed_of(&a[7]), content: unhex32(&a[8]) });
        }
        ("offer", 10) => {
            ...
            let intent = Intent::AcceptOffer {
                maker: account(&k),
                taker: unhex32(&a[9]),
                offer_asset: a[5].parse().unwrap_or_else(|_| die("give asset")),
                offer_amount: fixed_of(&a[6]),
                want_asset: a[7].parse().unwrap_or_else(|_| die("want asset")),
            ...
            let sig = k.sign(&payload).to_bytes();
            let mut e = Encoder::new();
            e.u32(chain).bytes(&auth.vm_id).u64(auth.valid_until_epoch).bytes(k.verifying_key().as_bytes()).bytes(&sig);
            wire::encode_intent(&mut e, &intent);
        ...
        ("accept", 6) => {
            let chain: u32 = a[3].parse().unwrap_or_else(|_| die("chain id"));
            let k = key(&a[4]);
            let raw: Vec<u8> = (0..a[5].len() / 2).map(|i| u8::from_str_radix(&a[5][i * 2..i * 2 + 2], 16).unwrap_or_else(|_| die("offer line"))).collect();
            ...
            let intent = wire::decode_intent(&mut d).unwrap_or_else(|_| die("offer line: bad intent"));
            let Intent::AcceptOffer { maker, taker, offer_asset, offer_amount, want_asset, want_amount } = &intent else { die("not an offer") };
            if *taker != account(&k) {
                die("this offer names a different taker");
            }
            eprintln!("offer from {}: they give {} of asset {} for {} of asset {}", hex(maker), offer_amount, offer_asset, want_amount, want_asset);
            let auth = Authorization { chain_id: chain, vm_id, valid_until_epoch: valid_until };
            let payload = match signed_bytes_as::<SwapState>(Scheme::Ed25519, &auth, &intent) {
                Signed::Message(m) => m,
                Signed::Prehash(h) => h.to_vec(),
            };
            let sig = k.sign(&payload).to_bytes();
            let mut e = Encoder::new();
            e.u8(OP_SUBMIT_MULTI).u32(chain).bytes(&vm_id).u64(valid_until).u8(2)
                .u8(Scheme::Ed25519.tag()).bytes(&maker_key).bytes(&maker_sig)
                .u8(Scheme::Ed25519.tag()).bytes(k.verifying_key().as_bytes()).bytes(&sig);
            wire::encode_intent(&mut e, &intent);
            let body = call(&a[2], e.finish());
            let mut d = Decoder::new(&body);
            let seq = d.u64().unwrap_or(0);
            ...
            let first = d.u8().unwrap_or(0);
            if first == 12 {
                die(&format!("rejected at seq {} (reject code {})", seq, d.u8().unwrap_or(0)));
            }
            println!("accepted: seq {} ({} receipt(s)) — the trade is done", seq, count);
        }
        ("reveal-submit", 7) => {
            ...
            let k = key(&a[4]);
            let id = account(&k);
            let kind: u8 = if zyn_custody::solana::pubkey(&a[5]).is_some() && !a[5].starts_with("utest") && !a[5].starts_with('t') { 1 } else { 0 };
            let epoch = status(&a[2], chain).epoch;
            let sig = k.sign(&read_challenge(chain, &id, epoch)).to_bytes();
            let mut e = Encoder::new();
            e.u8(OP_REVEAL).u32(chain).bytes(&id).u64(epoch).u8(Scheme::Ed25519.tag()).bytes(k.verifying_key().as_bytes()).bytes(&sig);
            e.u8(kind).u16(a[5].len() as u16).bytes(a[5].as_bytes()).bytes(&unhex32(&a[6]));
            call(&a[2], e.finish());
            println!("reveal accepted: the operator can now pay your exit to {}", a[5]);
        }
        ("record", 5) | ("record", 6) => {
            // Keep the exit proof: the record and its path to the anchored root.
            let chain: u32 = a[3].parse().unwrap_or_else(|_| die("chain id"));
            let k = key(&a[4]);
            let out = a.get(5).cloned().unwrap_or_else(|| format!("{}.exit.json", a[4]));
            let node = zynzapd::client::Node::new(&a[2], chain);
            let p = node.account_proof(&k).unwrap_or_else(|e| die(&e)).unwrap_or_else(|| die("this account has no record in the anchored snapshot"));
            let proof = zynzapd::exitproof::ExitProof { chain_id: chain, epoch: p.epoch, root: p.root, record: p.record, index: p.index, path: p.path, fetched_at: now() };
            if !proof.verify() {
                die("the node handed back a proof that does not open its own root — do not trust this node");
            }
            proof.save(std::path::Path::new(&out)).unwrap_or_else(|e| die(&e));
            println!("exit proof for epoch {} (root {}) kept in {}", proof.epoch, hex(&proof.root), out);
        }
        ("exit-proof", 3) => {
            let p = zynzapd::exitproof::ExitProof::load(std::path::Path::new(&a[2])).unwrap_or_else(|| die("not an exit proof file"));
            if !p.verify() {
        ...
        ("verify-exit", 3) => {
            let p = zynzapd::exitproof::ExitProof::load(std::path::Path::new(&a[2])).unwrap_or_else(|| die("not an exit proof file"));
            if p.verify() {
                println!("VALID: the record opens root {} (chain {}, epoch {})", hex(&p.root), p.chain_id, p.epoch);
        ...
        ("force-memo", 9) => {
            ...
            let to = unhex32(&a[6]);
            let asset: u32 = a[7].parse().unwrap_or_else(|_| die("asset id"));
            let amount = zynzapd::client::fixed_of(a.get(8).map(String::as_str).unwrap_or_else(|| die("amount"))).unwrap_or_else(|e| die(&e));
            let epoch = status(&a[2], chain).epoch;
            let intent = Intent::Transfer { from: account(&k), to, asset, amount };
            let frame = zynzapd::client::frame_submission(&k, chain, epoch, &intent);
            let memo = zyn_custody::memo::encode_forced(&frame).unwrap_or_else(|| die("the signed intent does not fit a memo"));
        ...
        ("status", 4) => {
            ...
            println!("ZEC.zy backing {}  pools {}  accounts {}", s.backing, s.pools, s.accounts);
            for h in &s.health {
                println!("  {:<16} scanned to {:<12} {}{}", h.0, h.1, if h.2 { "DOWN" } else { "ok" }, if h.3.is_empty() { String::new() } else { format!("  — {}", h.3) });
            }
        }
        ("bind", 7) => {
            let chain: u32 = a[3].parse().unwrap_or_else(|_| die("chain id"));
        ...
        ("withdraw", 8) => {
            let chain: u32 = a[3].parse().unwrap_or_else(|_| die("chain id"));
            let k = key(&a[4]);
            let zec: f64 = a[5].parse().unwrap_or_else(|_| die("amount in ZEC, e.g. 0.01"));
            let zat = (zec * 1e8).round() as i128;
            let (destination, _) = dest(&a[6], &a[7]);
            submit(&a[2], chain, &k, Intent::RequestWithdrawal { account: account(&k), asset: XZEC, amount: Fixed::raw(zat * ZAT), destination });
        }
        _ => {
