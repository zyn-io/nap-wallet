    ...
    match a.get(1).map(String::as_str) {
        Some("relay") => {
            let n: usize = a
                ...
                .unwrap_or_else(|| die("relay <participants>"));
            ...
            zyn_custody::dkg_net::net::serve(
                Arc::new(Mutex::new(zyn_custody::dkg_net::net::Relay::new(n))),
                listener,
            );
        }
        Some("join") if a.len() == 7 => {
            ...
            eprintln!(
                "zyn-dkg: joining as participant {} of {} (threshold {}) via {}",
                idx, participants, threshold, relay
            );
            let keys = zyn_custody::dkg_net::net::participate(relay, idx, threshold, participants)
                .unwrap_or_else(|e| die(&e));
            std::fs::create_dir_all(&out).unwrap_or_else(|e| die(&e.to_string()));
            zyn_custody::shares::save::<zyn_custody::ceremony::Zcash>(
                &out,
                &[(*keys.key_package.identifier(), keys.clone())],
            )
            .unwrap_or_else(|e| die(&e.to_string()));
            // The viewing key the settler/scanner needs, derived from the group.
            let group = keys.public_package.verifying_key();
            if let Some(fvk) =
                zyn_custody::ceremony::orchard_viewing_key(group, [0u8; 32], [0u8; 32])
            {
                let _ = std::fs::write(
                    out.join("viewing.hex"),
                    fvk.to_bytes()
                        .iter()
                        .map(|b| format!("{:02x}", b))
                        .collect::<String>(),
        ...
        Some("address") if a.len() == 4 => {
            ...
            let network = match a[3].as_str() {
                ...
                other => die(&format!(
                    "network must be mainnet or testnet, not {}",
                    other
                )),
            };
            let hex = std::fs::read_to_string(dir.join("viewing.hex"))
                .unwrap_or_else(|e| die(&format!("viewing.hex: {}", e)));
            let raw: Vec<u8> = hex
                .trim()
                .as_bytes()
                .chunks(2)
                .map(|c| {
                    u8::from_str_radix(std::str::from_utf8(c).unwrap_or("zz"), 16)
                        .unwrap_or_else(|_| die("viewing.hex is not hex"))
                })
                .collect();
            let bytes: [u8; 96] = raw
                .as_slice()
                .try_into()
                .unwrap_or_else(|_| die("viewing key is not 96 bytes"));
            let fvk = orchard::keys::FullViewingKey::from_bytes(&bytes)
                .unwrap_or_else(|| die("viewing key does not decode"));
