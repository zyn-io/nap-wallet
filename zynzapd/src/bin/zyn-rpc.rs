...
fn main() {
    ...
    eprintln!("zyn-rpc: chain {} relaying {} — POST http://{}/", chain, node_addr, listen);
    let cfg = Arc::new(Cfg { node: node_addr, chain, origin });
    for stream in listener.incoming().flatten() {
        let cfg = Arc::clone(&cfg);
        std::thread::spawn(move || { let _ = handle(&cfg, stream); });
    }
}
...
fn handle(cfg: &Cfg, mut s: TcpStream) -> std::io::Result<()> {
    let Some((method, body)) = read_request(&mut s) else { return Ok(()) };
    let cors = format!(
    ...
    match method.as_str() {
        ...
        "GET" => respond(&mut s, "200 OK", "application/json", &cors, discovery(cfg).to_string().as_bytes()),
        "POST" => {
            ...
            let status = if bytes.is_empty() { "204 No Content" } else { "200 OK" };
            respond(&mut s, status, "application/json", &cors, &bytes)
        }
        _ => respond(&mut s, "405 Method Not Allowed", "text/plain", &cors, b"POST JSON-RPC here"),
    }
}
...
fn discovery(cfg: &Cfg) -> Value {
    json!({
        ...
        "chain": cfg.chain,
        "methods": [
            "zyn_status", "zyn_pools", "zyn_assets", "zyn_quote", "zyn_anchors",
...
fn dispatch_body(cfg: &Cfg, body: &[u8]) -> Option<Value> {
    ...
    match parsed {
        Value::Array(items) if items.is_empty() => Some(err_obj(Value::Null, INVALID_REQUEST, "empty batch")),
        Value::Array(items) => {
            let out: Vec<Value> = items.into_iter().filter_map(|it| dispatch_one(cfg, it)).collect();
...
fn unhex(s: &str, what: &str) -> Result<Vec<u8>, (i64, String)> {
    ...
    if !s.len().is_multiple_of(2) {
        return Err((INVALID_PARAMS, format!("{}: odd number of hex digits", what)));
    }
    (0..s.len())
        .step_by(2)
        .map(|i| u8::from_str_radix(&s[i..i + 2], 16).map_err(|_| (INVALID_PARAMS, format!("{}: not hex", what))))
        .collect()
}
...
fn fixed_of(s: &str) -> Result<zyn_vm::fixed::Fixed, (i64, String)> {
    ...
    let whole: i128 = if whole.is_empty() { 0 } else { whole.parse().map_err(|_| bad("not a decimal"))? };
    let frac: i128 = if frac.is_empty() { 0 } else { format!("{:0<18}", frac).parse().map_err(|_| bad("not a decimal"))? };
    whole
    ...
    match method {
        ...
        "zyn_pools" => {
            ...
            Ok(json!(v.iter().map(|x| json!({
                "id": x.id,
                "asset0": x.asset0, "asset1": x.asset1,
        ...
        "zyn_assets" => {
            ...
            Ok(json!(v.iter().map(|a| json!({
                "id": a.id, "symbol": a.symbol, "supply": a.supply.to_string(),
                "lp_of": a.lp_of, "collection": a.collection,
                "content": a.content.map(|c| hex(&c)),
            })).collect::<Vec<_>>()))
        }
        "zyn_quote" => {
            let asset_in = want_u64(p, 0, "assetIn")? as u32;
            let path: Vec<u32> = param(p, 1, "path")
                .and_then(Value::as_array)
                .ok_or_else(|| (INVALID_PARAMS, "path must be an array of pool ids".to_string()))?
                .iter()
                .filter_map(|v| v.as_u64().map(|x| x as u32))
                .collect();
            let amount = fixed_of(&want_str(p, 2, "amount")?)?;
            let (out, fee, asset_out) = node_err(n.quote(asset_in, &path, amount))?;
            Ok(json!({ "out": out.to_string(), "fee": fee.to_string(), "assetOut": asset_out }))
        }
        "zyn_anchors" => {
            let limit = param(p, 0, "limit").and_then(Value::as_u64).unwrap_or(100).min(10_000) as u32;
        ...
        "zyn_offer" => {
            ...
            Ok(node_err(n.offer(id))?.as_ref().map(offer_json).unwrap_or(Value::Null))
        }
        "zyn_collections" => {
        ...
        "zyn_collection" => {
            let id = want_u64(p, 0, "id")? as u32;
            Ok(node_err(n.collection(id))?.as_ref().map(collection_json).unwrap_or(Value::Null))
        }
        ...
        "zyn_account" => {
            ...
            Ok(match r {
                None => Value::Null,
                Some(r) => json!({
                    "spendable": r.spendable.iter().map(|(a, v)| json!({ "asset": a, "amount": v.to_string() })).collect::<Vec<_>>(),
                    "exiting": r.exiting.iter().map(|(a, v, e)| json!({ "asset": a, "amount": v.to_string(), "epoch": e })).collect::<Vec<_>>(),
                    "unreleased": r.unreleased.iter().map(|(a, v, e)| json!({ "asset": a, "amount": v.to_string(), "epoch": e })).collect::<Vec<_>>(),
...
fn offer_json(o: &zynzapd::client::OfferView) -> Value {
    json!({
        ...
        "offerAsset": o.offer_asset, "offerAmount": o.offer_amount.to_string(),
        "wantAsset": o.want_asset, "wantAmount": o.want_amount.to_string(),
...
fn collection_json(c: &zynzapd::client::CollectionView) -> Value {
    json!({
        "id": c.id, "creator": hex(&c.creator), "symbol": c.symbol,
