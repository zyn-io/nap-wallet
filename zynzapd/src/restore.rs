...
fn sync_dir(path: &Path) -> Result<(), String> {
    File::open(path)
        ...
        .map_err(|e| format!("sync restore directory: {e}"))
}
...
fn private_file(path: &Path, bytes: &[u8]) -> Result<(), String> {
    ...
    let mut file = options
        .open(path)
        .map_err(|e| format!("create restore file: {e}"))?;
    file.write_all(bytes)
        .and_then(|_| file.sync_all())
        .map_err(|e| format!("sync restore file: {e}"))
}

pub(crate) fn export_file(dir: &Path, bytes: &[u8]) -> Result<PathBuf, String> {
    let path = dir.join(format!(
        "nap-wallet-backup-{:016x}.json",
        rand::random::<u64>()
    ));
...
pub(crate) fn pending(dir: &Path) -> bool {
...
pub(crate) fn recover(dir: &Path) -> Result<(), String> {
    ...
    let entries: Vec<Value> =
        serde_json::from_slice(&bytes).map_err(|e| format!("invalid restore journal: {e}"))?;
    for entry in entries {
        let target = Path::new(entry["target"].as_str().ok_or("invalid restore target")?);
        let old = match entry["backup"].as_str() {
            Some(path) => {
                Some(fs::read(path).map_err(|e| format!("read restore recovery copy: {e}"))?)
            }
            None => None,
        };
...
pub(crate) fn replace(dir: &Path, files: &[(PathBuf, Option<Vec<u8>>)]) -> Result<PathBuf, String> {
    ...
    builder
        .create(&backup)
        .map_err(|e| format!("create recovery directory: {e}"))?;
    let mut entries = Vec::new();
    for (i, (target, _)) in files.iter().enumerate() {
        if target.is_symlink() {
            return Err("refusing to replace a symlink during restore".into());
        }
    ...
    if let Err(error) = result {
        ...
        recover(dir).map_err(|rollback| {
            format!("{error}; rollback failed: {rollback}; restart required")
        })?;
        return Err(error);
    }
    Ok(backup)
...
mod tests {
    ...
    fn directory() -> PathBuf {
        let path =
            std::env::temp_dir().join(format!("nap-restore-test-{:016x}", rand::random::<u64>()));
    ...
    fn replace_retains_both_original_keys() {
        let dir = directory();
        let a = dir.join("wallet");
        let b = dir.join("zyn.key");
        private_file(&a, b"old-wallet").unwrap();
        private_file(&b, b"old-zyn").unwrap();
        let backup = replace(
            &dir,
            &[
                (a.clone(), Some(b"new-wallet".to_vec())),
                (b.clone(), Some(b"new-zyn".to_vec())),
            ],
        )
        .unwrap();
        assert_eq!(fs::read(a).unwrap(), b"new-wallet");
        assert_eq!(fs::read(b).unwrap(), b"new-zyn");
        assert_eq!(fs::read(backup.join("0")).unwrap(), b"old-wallet");
        assert_eq!(fs::read(backup.join("1")).unwrap(), b"old-zyn");
        assert!(!pending(&dir));
    }
    #[test]
    fn failed_install_rolls_back_already_replaced_key() {
        let dir = directory();
        let a = dir.join("wallet");
        let b = dir.join("absent/zyn.key");
        private_file(&a, b"old-wallet").unwrap();
        assert!(replace(
            &dir,
            &[
                (a.clone(), Some(b"new-wallet".to_vec())),
                (b, Some(b"new-zyn".to_vec()))
            ]
        )
        .is_err());
        ...
        fs::create_dir(dir.join("absent")).unwrap();
        recover(&dir).unwrap();
        assert!(!pending(&dir));
    }
    #[test]
    fn interrupted_install_is_recovered_idempotently() {
        ...
        private_file(&a, b"partial-new").unwrap();
        private_file(&saved, b"old-wallet").unwrap();
        private_file(
            &dir.join(JOURNAL),
            &serde_json::to_vec(&json!([{ "target": a, "backup": saved }])).unwrap(),
        )
        .unwrap();
        recover(&dir).unwrap();
        recover(&dir).unwrap();
