...
fn unhex(s: &str) -> Result<Vec<u8>, String> {
    if !s.len().is_multiple_of(2) {
        return Err("hex has an odd length".into());
    }
    (0..s.len())
        .step_by(2)
...
fn write_key(path: &str, bytes: &[u8]) -> Result<(), String> {
    ...
    let mut file = options
        .open(path)
        .map_err(|e| format!("cannot create wallet key: {}", e))?;
    file.write_all(bytes)
        .map_err(|e| format!("cannot write wallet key: {}", e))?;
    file.sync_all()
        .map_err(|e| format!("cannot sync wallet key: {}", e))
}
...
impl KeyMaterial {
    fn generate() -> Result<KeyMaterial, String> {
        ...
        Ok(KeyMaterial::Bip39 {
            entropy: words.to_entropy(),
    ...
    fn mnemonic(&self) -> Option<String> {
        match self {
            ...
            KeyMaterial::Bip39 { entropy, .. } => {
                Mnemonic::from_entropy(entropy).ok().map(|m| m.to_string())
            }
        }
    }
    ...
    fn account(&self) -> u32 {
        match self {
            KeyMaterial::Raw(_) => 0,
            KeyMaterial::Bip39 { account, .. } => *account,
        }
    }
    ...
    fn seed(&self) -> Result<Vec<u8>, String> {
        match self {
            ...
            KeyMaterial::Bip39 {
                ...
                let words = Mnemonic::from_entropy(entropy)
                    .map_err(|e| format!("invalid wallet entropy: {}", e))?;
        ...
        match self {
            ...
            KeyMaterial::Bip39 { account, .. } => {
                let id = zip32::AccountId::try_from(*account)
                    .map_err(|_| "account must be below 2^31".to_string())?;
    ...
    fn zyn_signing_key(&self, account: u32) -> Result<SigningKey, String> {
        ...
        if account != *wallet_account {
            return Err("Zyn derivation account does not match the wallet account".into());
        }
        ...
        hk.expand(&info, &mut seed)
            .map_err(|_| "cannot derive Zyn key".to_string())?;
    ...
    fn decode(bytes: &[u8]) -> Result<KeyMaterial, String> {
        ...
        let v: Value = serde_json::from_slice(bytes).map_err(|_| {
            "wallet key is neither a legacy 32-byte key nor a Nap key record".to_string()
        })?;
        if v.get("format").and_then(Value::as_str) != Some(KEY_FORMAT)
            || v.get("version").and_then(Value::as_u64) != Some(KEY_VERSION)
        {
        ...
        let entropy = unhex(
            v.get("entropy")
                ...
                .ok_or("wallet key has no entropy")?,
        )?;
        ...
        let account =
            u32::try_from(account).map_err(|_| "wallet account is too large".to_string())?;
...
impl std::fmt::Debug for ZynKeySource {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Derived { version, account } => f
...
impl ZynKeySource {
    pub fn derived(account: u32) -> Self {
        Self::Derived {
            version: ZYN_DERIVATION_VERSION,
            account,
        }
    }
        ...
        match v.get("source").and_then(Value::as_str) {
            Some("bip39-hkdf") => {
                let version = v
                    ...
                    .ok_or("Zyn derivation has no version")?;
                if version != ZYN_DERIVATION_VERSION {
                    return Err("unsupported Zyn derivation version".into());
                }
                if v.get("domain").and_then(Value::as_str) != Some("nap.zyn.ed25519.v1")
                    || v.get("network_scoped").and_then(Value::as_bool) != Some(false)
                {
            ...
            Some("legacy-ed25519") => {
                let bytes = unhex(
                    v.get("seed")
                        .and_then(Value::as_str)
                        .ok_or("legacy Zyn backup has no seed")?,
                )?;
                Ok(Self::Legacy {
                    seed: bytes
                        .try_into()
                        .map_err(|_| "legacy Zyn seed must be 32 bytes")?,
                })
            }
...
impl WalletBackup {
    ...
    pub fn from_mnemonic(
        ...
        Ok(WalletBackup {
            ...
            network,
            birthday,
            zyn: Some(ZynKeySource::derived(account)),
        })
    }

    pub fn parse(text: &str) -> Result<WalletBackup, String> {
        let v: Value =
            serde_json::from_str(text).map_err(|e| format!("backup is not JSON: {}", e))?;
        let version = v.get("version").and_then(Value::as_u64);
        if v.get("format").and_then(Value::as_str) != Some(BACKUP_FORMAT)
            || !matches!(version, Some(KEY_VERSION | BACKUP_VERSION))
        {
        ...
        let birthday = v
            .get("birthday")
            .and_then(Value::as_u64)
            .ok_or("backup has no birthday")?;
        let source = v
            .get("source")
            .and_then(Value::as_str)
            .ok_or("backup has no key source")?;
        let material = match source {
            "bip39" => {
                let words = v
                    .get("mnemonic")
                    .and_then(Value::as_str)
                    .ok_or("backup has no recovery phrase")?;
                let passphrase = v.get("passphrase").and_then(Value::as_str).unwrap_or("");
                let account = v.get("account").and_then(Value::as_u64).unwrap_or(0);
                KeyMaterial::from_mnemonic(
                    words,
                    passphrase,
                    u32::try_from(account).map_err(|_| "backup account is too large")?,
                )?
            }
            "raw-orchard" => {
                let bytes = unhex(
                    v.get("spending_key")
                        .and_then(Value::as_str)
                        .ok_or("backup has no spending key")?,
                )?;
                KeyMaterial::Raw(
                    bytes
                        .try_into()
                        .map_err(|_| "spending key must be 32 bytes")?,
                )
            }
        ...
        if let Some(ZynKeySource::Derived {
            account: zyn_account,
            ..
        }) = zyn
        {
    ...
    pub fn zyn(&self) -> Option<ZynKeySource> {
        self.zyn.clone()
    }
...
impl State {
    ...
    fn save(&self, wallet: &str) -> Result<(), String> {
        ...
        out.push(if self.network == Network::MainNetwork {
            NET_MAINNET
        } else {
            NET_TESTNET
        });
    ...
    fn load(wallet: &str) -> Result<Option<State>, String> {
        let Ok(b) = std::fs::read(State::path(wallet)) else {
            return Ok(None);
        };
        let bad = || "state file is not a zyn-wallet state".to_string();
        if b.get(..8).ok_or_else(bad)? != STATE_MAGIC {
            return Err(bad());
        }
        ...
        Ok(Some(State {
            ...
            birthday,
            stores: PoolStores {
                orchard: Arc::new(Mutex::new(orchard)),
                ironwood: Arc::new(Mutex::new(ironwood)),
            },
        }))
    }
    ...
    fn seeded(client: &Client, network: Network, birthday: u64) -> Result<State, String> {
        ...
        let seed = |pool: ValuePool| -> Result<Arc<Mutex<NoteStore>>, String> {
            ...
            let store =
                NoteStore::from_frontier(&frontier, seed_height).map_err(|e| format!("{:?}", e))?;
...
impl Wallet {
    ...
    fn install(
        ...
        if expected_network.is_some_and(|n| n != network) {
            return Err(format!(
                "this backup is for {}, but the block server is {}",
                network_name(expected_network.unwrap()),
    ...
    pub fn open(path: &str, client: Client) -> Result<Wallet, String> {
        ...
        let state = match State::load(path)? {
            Some(s) if s.network != network => {
                return Err(format!(
                    "this wallet has been syncing {}; the block server is {}",
    ...
    pub fn address(&self) -> String {
        ...
        VaultKeys::from_full_viewing_key(self.fvk.clone()).address(
            0,
            <Network as zcash_protocol::consensus::Parameters>::network_type(&self.state.network),
        )
    }
    ...
    pub fn shield_transparent(&mut self, mut on: impl FnMut(Event)) -> Result<String, String> {
        ...
        Err(if tried.is_empty() {
            "no pool could carry the sweep".into()
        } else {
            tried.join(" | ")
        })
    }
            ...
            if let Ok(s) = self.state.stores.of(pool).lock() {
                for h in s.held() {
                    ...
                    out.push(NoteView {
                        ...
                        zatoshi: h.value(),
                        height: h.height,
                        txid_hex: t.iter().map(|b| format!("{:02x}", b)).collect(),
                    });
                }
            }
        }
    ...
    pub fn sync(&mut self, mut on: impl FnMut(Event)) -> Result<u64, String> {
        ...
        if from > to {
            return Ok(self
                ...
                .stores
                .synced_to()
                .unwrap_or(self.state.birthday.saturating_sub(1)));
        }
        let mut height = from;
        ...
        while height <= to {
            ...
            for block in &blocks {
                if block.height != height {
                    return Err(format!(
                        "server sent block {} where {} was expected",
                        block.height, height
                    ));
                }
    ...
    fn scan_block(
        ...
        for t in &block.txs {
            ...
            let mut our_actions = full
                .as_ref()
                .map(|s| s.actions.iter().filter(|a| a.ours.is_some()))
                .into_iter()
                .flatten();
            for (pool, list) in [
                ...
                for out in list {
                    let act = out
                        .to_action()
                        .ok_or("malformed output in a compact block")?;
                    if let Some(spent) = store.spend_nullifier(&act.nullifier(), &self.fvk) {
                        on(Event::Spent {
                            pool,
                            zatoshi: spent.value(),
                            from_height: spent.height,
                            txid_hex: txid_hex(&t.txid),
                            height: block.height,
                        });
                    }
                    let ours = hits
                        .iter()
                        .any(|h| h.tx_index == t.index && h.pool == pool && h.cmx == out.cmx);
                    let pos = store.append(&act.cmx(), ours);
                    if ours {
                        let a = our_actions
                            .find(|a| a.pool == pool && a.cmx == act.cmx())
                            .ok_or("a compact hit did not decrypt in the full transaction")?;
                        if let (Some(note), Some(pos)) = (a.ours, pos) {
                            on(Event::Received {
                                ...
                                height: block.height,
                                txid_hex: txid_hex(&t.txid),
                                memo: a.memo.as_deref().map(memo_text).unwrap_or_default(),
                            });
    ...
    pub fn send(
        ...
        if let Some(memo) = memo {
            if memo.len() > 512 {
                return Err("memo longer than 512 bytes".into());
            }
    ...
    pub fn send_with_memo(
        ...
        let to = parse_destination(to, network)
            .ok_or_else(|| format!("not a {} address", network_name(network)))?;
        if zatoshi == 0 {
        ...
        for pool in [ValuePool::Ironwood, ValuePool::Orchard] {
            ...
            if matches!(to, Destination::Shielded(_))
        ...
        p.finalize_io(sighash, rand::rngs::OsRng)
            .map_err(|e| e.to_string())?;
        p.prove(&payout::proving_key(version), rand::rngs::OsRng)
            .map_err(|e| e.to_string())?;
        let ask = SpendAuthorizingKey::from(&self.sk);
        if payout::sign_with_key(&mut p, sighash, &ask, rand::rngs::OsRng) == 0 {
            return Err("nothing was signed".into());
        }
        let sealed = p
        ...
        let txid = self
            .client
            .send(&sealed.bytes)
            .map_err(|e| format!("the network refused it: {}", e))?;
        ...
        // nullifiers and confirm it on the next sync.
        {
            let mut store = self
                ...
                .lock()
                .map_err(|_| "state lock")?;
            for n in &sealed.spent {
                store.spend(n.position);
            }
        }
        self.state.save(&self.path)?;
        Ok(txid)
...
mod recovery_tests {
    ...
    fn bip39_uses_the_standard_zip32_orchard_account_path() {
        ...
        assert_eq!(
            hex(material
                .spending_key(Network::MainNetwork)
                .unwrap()
                .to_bytes()),
        ...
        assert_eq!(
            hex(material
                .spending_key(Network::TestNetwork)
                .unwrap()
                .to_bytes()),
    ...
    fn portable_backup_carries_every_restore_parameter() {
        let backup =
            WalletBackup::from_mnemonic(Network::TestNetwork, 4_321_000, WORDS, "phrase pass", 3)
                .unwrap();
        ...
        assert_eq!(
            backup
                .material
                .spending_key(backup.network)
                .unwrap()
                .to_bytes(),
            restored
                .material
                .spending_key(restored.network)
                .unwrap()
                .to_bytes()
        );
        assert!(matches!(
            restored.zyn(),
            Some(ZynKeySource::Derived {
                version: 1,
                account: 3
            })
        ));
    ...
    fn backup_parser_refuses_unknown_versions_and_bad_mnemonics() {
        ...
        assert!(WalletBackup::parse(&unknown.to_string())
            .err()
            .unwrap()
            .contains("unsupported"));
        assert!(
            WalletBackup::from_mnemonic(Network::MainNetwork, 1, "abandon abandon", "", 0).is_err()
        );
    }
    ...
    fn zyn_derivation_vectors_are_network_independent() {
        let vectors = [
            (
                ...
                "308ab8b209813f5912287682b50950d62782abc61507f0a80abafd0f7a33a7a6",
                "b85db260ec3a7c0a22c19c1f3380bfc75599c0ea4eeeeda69177ab12f9da56ea",
            ),
            (
                "nap passphrase",
                7,
                "f4e1b20f8a0cd2e19ae9d85ce3057cbb13863be3630e87972afce0b14c513c2e",
                "276237e6804911ecd6d44c3d170ac67ff8dc8abf87cf423489a71dddf68857eb",
                "4c976ef0d248340b910e246439c3139911f1752e6ba1d3c198f41071e4503604",
            ),
        ];
