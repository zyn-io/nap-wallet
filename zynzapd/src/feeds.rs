...
impl Source {
    ...
    pub fn parse(s: &str) -> Option<Source> {
        ...
        Some(match kind.trim().to_ascii_lowercase().as_str() {
            ...
            "chainlink" => {
                let (feed, rpc) = id.split_once('@')?;
                Source::Chainlink(rpc.to_string(), feed.to_string(), 86_400)
            }
            _ => return None,
        })
    }
...
pub fn default_map() -> BTreeMap<String, Vec<Source>> {
    ...
    for s in [
        "AAPL", "TSLA", "NVDA", "MSFT", "AMZN", "GOOGL", "META", "COIN", "MSTR", "HOOD", "NFLX",
        "AMD", "SPY", "QQQ", "IWM", "GLD", "SLV", "TLT", "IBIT", "ETHA", "VOO",
    ] {
    ...
    for (_, sym) in tokenized_stocks().keys() {
        m.entry(sym.clone())
            .or_insert_with(|| vec![nq(sym), ya(sym)]);
    }
...
pub fn tokenized_stocks() -> BTreeMap<(String, String), String> {
    TOKENIZED
        ...
        .filter_map(|l| {
            let mut it = l.split('\t');
            Some((
                (it.next()?.to_string(), it.next()?.to_string()),
                it.next()?.to_string(),
            ))
        })
        .collect()
    ...
    for ((issuer, sym), id) in &cg {
        ...
        if issuer == "RH" {
            ...
            if let Some((token, _)) = rh.get(sym) {
                v.push(Source::Scaled(
                    rpc.to_string(),
                    token.clone(),
                    Box::new(Source::Nasdaq(sym.clone())),
                ));
                v.push(Source::Scaled(
                    rpc.to_string(),
                    token.clone(),
                    Box::new(Source::Yahoo(sym.clone())),
                ));
            }
        } else {
            v.push(Source::CoinGecko(id.clone()));
...
pub fn apply_overrides(map: &mut BTreeMap<String, Vec<Source>>, spec: &str) -> Result<(), String> {
    for entry in spec.split(',').map(str::trim).filter(|e| !e.is_empty()) {
        let (sym, srcs) = entry
            .split_once('=')
            .ok_or_else(|| format!("bad feed entry: {}", entry))?;
        let list: Result<Vec<Source>, String> = srcs
            .split('|')
            .map(|s| Source::parse(s).ok_or_else(|| format!("bad source: {}", s)))
            .collect();
...
pub fn parse_coingecko(body: &str, id: &str) -> Result<f64, String> {
    ...
    v.get(id)
        ...
        .and_then(|a| a.as_f64())
        .ok_or_else(|| format!("no usd price for {}", id))
}
...
pub fn parse_yahoo(body: &str) -> Result<f64, String> {
    let v: serde_json::Value = serde_json::from_str(body).map_err(|e| e.to_string())?;
    v.pointer("/chart/result/0/meta/regularMarketPrice")
        .and_then(|p| p.as_f64())
        .ok_or_else(|| "no regularMarketPrice".into())
}

/// One `eth_call` over JSON-RPC, returning the raw return data.
pub fn eth_call(
    get_post: &dyn Fn(&str, &str) -> Result<String, String>,
    rpc: &str,
    to: &str,
    data: &str,
...
impl Fetcher for Http {
    fn prime(&self, ids: &[String]) {
        ...
        for chunk in ids.chunks(50) {
            ...
            let body = ureq::AgentBuilder::new()
                ...
                .build()
                .get(&url)
                .set("User-Agent", "Mozilla/5.0 (zynzapd-feeds)")
            ...
            if let Ok(v) = serde_json::from_str::<serde_json::Value>(&body) {
                for id in chunk {
                    if let Some(px) = v
                        ...
                        .and_then(|a| a.as_f64())
                    {
                        fresh.insert(id.clone(), px);
                    }
                }
            }
        }
        if let Ok(mut c) = self.coingecko.lock() {
            *c = fresh;
        }
    }
    ...
    fn fetch(&self, source: &Source) -> Result<f64, String> {
        if let Source::CoinGecko(id) = source {
            if let Some(px) = self.coingecko.lock().ok().and_then(|c| c.get(id).copied()) {
                return Ok(px);
            }
        }
        ...
        match source {
            Source::Chainlink(rpc, feed, heartbeat) => {
                ...
                let dec = eth_call(&post, rpc, feed, "0x313ce567")
                    .and_then(|r| word_i128(&r, 0))
                    .unwrap_or(8) as i32;
                ...
                if now.saturating_sub(updated) > heartbeat * 3 {
                    return Err(format!(
                        "stale: updated {} s ago",
                ...
                if answer <= 0 {
                    return Err("non-positive answer".into());
                }
            ...
            Source::Scaled(rpc, token, inner) => {
                ...
                if !(0.1..=100.0).contains(&mult) {
                    return Err(format!("implausible multiplier {}", mult));
                }
                Ok(share * mult)
            }
            Source::Coinbase(sym) => parse_coinbase(&get(&format!(
                "https://api.coinbase.com/v2/prices/{}-USD/spot",
                sym
            ))?),
            Source::CoinGecko(id) => parse_coingecko(
                &get(&format!(
                    "https://api.coingecko.com/api/v3/simple/price?ids={}&vs_currencies=usd",
                    id
                ))?,
            ...
            Source::Nasdaq(sym) => {
                ...
                let stocks = get(&format!(
                    "https://api.nasdaq.com/api/quote/{}/info?assetclass=stocks",
                    sym
                ))
                .and_then(|b| parse_nasdaq(&b));
                match stocks {
                    Ok(x) => Ok(x),
                    Err(_) => parse_nasdaq(&get(&format!(
                        "https://api.nasdaq.com/api/quote/{}/info?assetclass=etf",
                        sym
                    ))?),
                }
            }
            Source::Yahoo(sym) => parse_yahoo(&get(&format!(
                "https://query1.finance.yahoo.com/v8/finance/chart/{}?range=1d&interval=1d",
                sym
            ))?),
            Source::Frankfurter(cur) => parse_frankfurter(&get(&format!(
...
impl<F: Fetcher> Feeds<F> {
    ...
    pub fn prices(&mut self, symbols: &[String]) -> (BTreeMap<String, f64>, Vec<String>) {
        ...
        let ids: Vec<String> = symbols
            ...
            .filter_map(|s| match s {
                Source::CoinGecko(id) => Some(id.clone()),
                _ => None,
            })
            .collect();
        self.fetcher.prime(&ids);
        for sym in symbols {
            let Some(sources) = self.map.get(sym) else {
                continue;
            };
            ...
            let Some(px) = aggregate(samples, self.cfg.max_spread) else {
                if n > 0 {
                    errors.push(format!("{}: sources disagree", sym))
                }
                continue;
            };
            if n == 1 {
                if let Some(prev) = self.last.get(sym) {
                    if ((px - prev) / prev).abs() > self.cfg.max_jump {
                        errors.push(format!(
                            "{}: single source jumped {:.1}%, ignored",
                            sym,
                            100.0 * (px - prev) / prev
                        ));
                        continue;
    ...
    pub fn asset_references(
        &self,
        pending: &[(swapvm::types::AssetId, Side)],
        prices: &BTreeMap<String, f64>,
    ) -> Vec<(swapvm::types::AssetId, f64)> {
        let zec = match prices.get("ZEC") {
            Some(z) if *z > 0.0 => *z,
            _ => return Vec::new(),
        };
        pending
...
pub fn pending_assets(state: &SwapState) -> Vec<(swapvm::types::AssetId, Side)> {
    state
        ...
        .filter_map(|(id, t)| {
            let v = t.vault?;
            if *id == swapvm::types::XZEC
                || !t.is_divisible()
                || state.find_pool(swapvm::types::XZEC, *id).is_some()
            {
                return None;
            }
            Some((
                *id,
                (
                    String::from_utf8_lossy(t.symbol.as_bytes()).to_string(),
                    v.origin,
                ),
            ))
        })
...
pub fn pool_pairs(state: &SwapState) -> Vec<(PoolId, Side, Side)> {
    let side = |a: swapvm::types::AssetId| -> Side {
        state
            .tokens
            .get(&a)
            .map(|t| {
                (
                    String::from_utf8_lossy(t.symbol.as_bytes()).to_string(),
                    t.vault.map(|v| v.origin).unwrap_or(0),
                )
            })
            .unwrap_or_default()
    };
    state
        .pools
        .iter()
        .map(|(id, p)| (*id, side(p.asset0), side(p.asset1)))
        .collect()
}

/// One cycle: price what the pools need and post what moved. Returns the
/// number of references posted.
pub fn cycle<F: Fetcher>(
    feeds: &mut Feeds<F>,
    shared: &Arc<Mutex<Node<SwapState>>>,
    now: u64,
    health: &Arc<Mutex<FeedHealth>>,
) -> Result<u64, String> {
    let (pairs, pending) = {
        let n = shared.lock().map_err(|_| "node lock poisoned")?;
        (pool_pairs(n.state()), pending_assets(n.state()))
    };
    let mut symbols: Vec<String> = pairs
        .iter()
        .flat_map(|(_, a, b)| [a, b])
        .filter_map(|s| base_symbol_from(&s.0, s.1, &feeds.map))
        .collect();
    symbols.extend(
        pending
            .iter()
            .filter_map(|(_, s)| base_symbol_from(&s.0, s.1, &feeds.map)),
    );
    if !pending.is_empty() {
        symbols.push("ZEC".to_string());
    }
    symbols.sort();
    symbols.dedup();
    let (prices, errors) = feeds.prices(&symbols);
    for e in &errors {
        eprintln!("zynzapd: feed: {}", e);
    }
    let refs = feeds.references(&pairs, &prices);
    let mut posted = 0;
    for (pool, price) in refs {
        if !feeds.due(pool, price, now) {
            continue;
        }
        ...
        if step.rejected() {
            eprintln!(
                "zynzapd: feed: reference for pool {} refused: {:?}",
                crate::client::hex(&pool),
                step.receipts
            );
            continue;
        }
        let fee = step
            ...
            .iter()
            .find_map(|r| match r {
                swapvm::tx::Receipt::ReferenceUpdated { fee_bps, .. } => Some(*fee_bps),
                _ => None,
            })
            .unwrap_or(0);
        eprintln!(
            "zynzapd: feed: pool {} reference {} (fee now {} bps)",
            crate::client::hex(&pool),
            fx,
            fee
        );
    ...
    for (asset, price) in feeds.asset_references(&pending, &prices) {
        ...
        let due = match last {
            None => true,
            Some((p, at)) => {
                ((price - p) / p).abs() * 10_000.0 > feeds.cfg.move_bps as f64
                    || now.saturating_sub(at) >= feeds.cfg.refresh_secs
            }
        };
        if !due {
            continue;
        ...
        if step.rejected() {
            eprintln!(
                "zynzapd: feed: price for asset {} refused: {:?}",
                crate::client::hex(&asset),
                step.receipts
            );
            continue;
        }
        eprintln!(
            "zynzapd: feed: asset {} priced at {} per ZEC.zy (no market yet)",
            crate::client::hex(&asset),
            fx
        );
    ...
    if prices.is_empty() && !symbols.is_empty() {
        return Err(errors
            .last()
            .cloned()
            .unwrap_or_else(|| "no source answered".into()));
    }
    Ok(posted)
}
...
mod tests {
    ...
    impl Fetcher for Fake {
        fn fetch(&self, source: &Source) -> Result<f64, String> {
            let key = match source {
                ...
                | Source::Nasdaq(s)
                | Source::Yahoo(s)
                | Source::Frankfurter(s) => format!("{}:{}", source.name(), s),
                Source::Constant(x) => return Ok(*x),
                Source::Chainlink(_, feed, _) => format!("chainlink:{}", feed),
                Source::Scaled(_, token, inner) => {
                    let base = self.fetch(inner)?;
                    return Ok(base
                        * self
                            .0
                            .get(&format!("mult:{}", token))
                            .cloned()
                            .unwrap_or(Ok(1.0))?);
                }
            };
    ...
    fn zyn_symbols_map_to_market_symbols() {
        ...
        assert_eq!(
            base_symbol("TSLAx.zy", &m).as_deref(),
            Some("X:TSLA"),
            "an xStocks token is the token"
        );
        assert_eq!(
            base_symbol("bAAPL.zy", &m).as_deref(),
            Some("AAPL"),
            "an unknown decoration falls back to the share"
        );
        ...
        assert_eq!(
            base_symbol("ART", &m),
            None,
            "a token launched here has no market"
        );
    ...
    fn the_median_is_taken_and_disagreement_refuses() {
        ...
        assert_eq!(
            aggregate(vec![100.0, 120.0], 0.05),
            None,
            "20% apart is not a price"
        );
    ...
    fn parsers_read_each_source() {
        assert_eq!(
            parse_coinbase(r#"{"data":{"base":"BTC","currency":"USD","amount":"63000.12"}}"#)
                .unwrap(),
            63000.12
        );
        assert_eq!(
            parse_coingecko(r#"{"zcash":{"usd":41.7}}"#, "zcash").unwrap(),
            41.7
        );
        ...
        assert_eq!(
            parse_yahoo(
                r#"{"chart":{"result":[{"meta":{"regularMarketPrice":412.5}}],"error":null}}"#
            )
            .unwrap(),
            412.5
        );
        assert_eq!(
            parse_frankfurter(r#"{"amount":1.0,"base":"EUR","rates":{"USD":1.0842}}"#).unwrap(),
            1.0842
        );
    }
    ...
    fn a_pool_reference_is_asset1_per_asset0() {
        ...
        let refs = f.references(
            &[
                (
                    swapvm::types::legacy_id(1),
                    ("ZEC.zy".into(), 0),
                    ("SOL.zy".into(), 0),
                ),
                (
                    swapvm::types::legacy_id(2),
                    ("ART".into(), 0),
                    ("ZEC.zy".into(), 0),
                ),
            ],
        ...
        assert_eq!(
            refs.len(),
            1,
            "a pool with an unpriceable side gets no reference"
        );
    ...
    fn a_lone_source_may_not_jump_and_posting_is_by_move_or_age() {
        ...
        assert!(
            !p.contains_key("OUSG"),
            "a 40% jump from one source is not believed"
        );
        assert!(errors[0].contains("jumped"));

        let pool = swapvm::types::legacy_id(1);
        assert!(f.due(pool, 0.2, 1_000));
        f.posted.insert(pool, (0.2, 1_000));
        assert!(
            !f.due(pool, 0.20001, 1_010),
            "a hair of movement is not worth a post"
        );
        assert!(f.due(pool, 0.2003, 1_010), "15 bps is");
        assert!(f.due(pool, 0.2, 1_000 + 600), "and so is age");
    }
    ...
    fn robinhood_tokens_are_priced_as_tokens_not_shares() {
        ...
        assert!(
            m["RH:AAOI"].len() >= 2,
            "no feed; token price and scaled share sources"
        );
        assert_eq!(
            base_symbol_from("AAPL.zy", zyn_bridge::evm::ORIGIN_ROBINHOOD_CHAIN, &m).as_deref(),
            Some("RH:AAPL")
        );
        assert_eq!(
            base_symbol_from("AAPL.zy", 0, &m).as_deref(),
            Some("AAPL"),
            "the bare share otherwise"
        );
        assert_eq!(
            base_symbol_from("SOL.zy", zyn_bridge::evm::ORIGIN_ROBINHOOD_CHAIN, &m).as_deref(),
            Some("SOL"),
            "not a stock: falls through"
        );
    }
    ...
    fn issuer_decorations_name_the_token() {
        ...
        assert_eq!(
            base_symbol_from("TSLAx.zy", 0, &m).as_deref(),
            Some("X:TSLA")
        );
        assert_eq!(
            base_symbol_from("AAPLon.zy", 0, &m).as_deref(),
            Some("ON:AAPL")
        );
        assert_eq!(
            base_symbol_from("AAPL.d.zy", 0, &m).as_deref(),
            Some("D:AAPL")
        );
        assert_eq!(
            base_symbol_from("NVDA.zy", 0, &m).as_deref(),
            Some("NVDA"),
            "undecorated is the share"
        );
        ...
        assert_eq!(
            m["RH:AAPL"].len(),
            4,
            "feed, coingecko, and two scaled share sources"
        );
    }
    ...
    fn a_chainlink_round_decodes() {
        ...
        let post = |_: &str, _: &str| -> Result<String, String> {
            Ok(format!(
        ...
        assert_eq!(r.len(), 160);
        let bad = |_: &str, _: &str| -> Result<String, String> {
            Ok(
    ...
    fn overrides_replace_a_symbol() {
        ...
        apply_overrides(
            &mut m,
            "TSLA=yahoo:TSLA, GOLD=coingecko:pax-gold|coinbase:PAXG",
        )
        .unwrap();
