...
mod tests {
    ...
    fn committed(batch: &[SequencedIntent]) -> Vec<(u64, Vec<u8>)> {
        batch
            ...
            .map(|si| {
                ...
                (
                    si.seq,
                    zyn_vm::verify::encode_authorized::<SwapState>(&authorized),
                )
            })
            .collect()
    }
    ...
    fn the_output_names_the_program_that_ran() {
        let s = chain();
        let out =
            Output::decode(&run(&encode_input(s.state_root(), &s, &batch(&s, s.seq))).unwrap())
                .unwrap();
        assert_eq!(out.vm_id, vm_id());
    ...
    fn the_guest_reverifies_a_delegated_action_from_committed_bytes() {
        ...
        let token = zyn_vm::asset_address(crate::types::ADDRESS_SCOPE_V1, &account, b"CAT");
        let pool = zyn_vm::pool_address(crate::types::ADDRESS_SCOPE_V1, &XZEC, &token);
        let quote = crate::vm::quote(&state, XZEC, &[pool], amount).unwrap();
        ...
        delegation.allowed_pools = alloc::vec![pool];
        delegation.max_per_action = alloc::vec![zyn_vm::session::AssetLimit {
            asset: XZEC,
            amount
        }];
        ...
        let tape =
            encode_committed_input(state.state_root(), &state, &[(state.seq() + 1, tampered)]);
